...
declare -a tags=("v2.5" "v2.4" "v2.3" "v2.2" "v2.8" "v3.0" "v3.1" "v3.2" "v3.3" "v3.4" "rocksdb-3.5.1" "rocksdb-3.6.2" "rocksdb-3.7" "rocksdb-3.8" "rocksdb-3.9")
...
do
   ...
   make clean
   make ldb -j32
   $script_copy_dir/generate_random_db.sh $input_data_path $test_dir/$tag
done
...
$script_copy_dir/generate_random_db.sh $input_data_path $compare_base_db_dir

for tag in "${tags[@]}"
do
   echo Opening DB from "$tag" ...
   $script_copy_dir/verify_random_db.sh $test_dir/$tag $compare_base_db_dir
done

echo Building $checkout_flag with release
